...
async function load(){try{const s=await (await fetch('/api/settings')).json();$('serverUrl').value=s.server_url||'';$('tailscaleEnabled').checked=!!s.tailscale_enabled;$('tailscaleName').value=s.tailscale_name||'epf-frame';$('tailscaleNote').textContent=s.tailscale_compiled?'':'Tailscale support is not included in this firmware.';if(s.wifi_ssid)$('ssid').value=s.wifi_ssid;const st=await (await fetch('/api/status')).json();isCaptive=!!st.captive;renderStatus(st);if(isCaptive)scan()}catch(e){setBadge('statusMode','Unavailable','bad')}}
