...
.password-toggle{display:flex;align-items:center;gap:.45rem;margin-top:.45rem;font-size:.9rem;font-weight:400}.password-toggle input{width:auto;margin:0}
...
<label for="wifiPassword">Wi-Fi password</label><input id="wifiPassword" type="password" autocomplete="off"><label class="password-toggle"><input id="showWifiPassword" type="checkbox"> Show password</label>
</fieldset>
...
<label for="adminPassword">Change admin password</label><input id="adminPassword" type="password" autocomplete="new-password" minlength="8" maxlength="16" placeholder="Leave blank to keep current"><label class="password-toggle"><input id="showAdminPassword" type="checkbox"> Show password</label>
</fieldset>
...
<label for="tailscaleKey">Provisioning auth key</label><input id="tailscaleKey" type="password" autocomplete="off" placeholder="Leave blank to keep current"><label class="password-toggle"><input id="showTailscaleKey" type="checkbox"> Show key</label>
...
function bindPasswordToggle(toggleId,inputId){const toggle=$(toggleId),input=$(inputId);if(toggle&&input)toggle.onchange=()=>{input.type=toggle.checked?'text':'password'}}
bindPasswordToggle('showWifiPassword','wifiPassword');bindPasswordToggle('showAdminPassword','adminPassword');bindPasswordToggle('showTailscaleKey','tailscaleKey');
function authHeaders(){let p=sessionStorage.getItem('epfAdminPassword');if(!p){p=prompt('Admin password (cancel for no password)');if(p)sessionStorage.setItem('epfAdminPassword',p)}return p?{Authorization:'Basic '+btoa('admin:'+p)}:{}}
function authFetch(url,options={}){return fetch(url,{...options,headers:{...(options.headers||{}),...authHeaders()}})}
...
async function refreshStatus(){try{renderStatus(await (await authFetch('/api/status')).json())}catch(e){setBadge('statusMode','Unavailable','bad')}}
async function scan(){try{const r=await fetch('/scan');if(r.status===202){setTimeout(scan,800);return}const a=await r.json();if(!a.length){$('ssid').innerHTML='<option value="">No networks found</option>';$('ssidManual').style.display='block';return}$('ssid').innerHTML='<option value="">Choose a network</option>'+a.map(n=>`<option value="${n.name.replaceAll('&','&amp;').replaceAll('"','&quot;')}">${n.name} (${n.rssi})</option>`).join('');if(savedSsid)$('ssid').value=savedSsid}catch(e){$('ssid').innerHTML='<option value="">Network scan unavailable</option>';$('ssidManual').style.display='block'}}
async function load(){try{const s=await (await authFetch('/api/settings')).json();$('serverUrl').value=s.server_url||'';$('tailscaleEnabled').checked=!!s.tailscale_enabled;$('tailscaleName').value=s.tailscale_name||'epf-frame';$('tailscaleNote').textContent=s.tailscale_compiled?'':'Tailscale support is not included in this firmware.';savedSsid=s.wifi_ssid||'';const st=await (await authFetch('/api/status')).json();isCaptive=!!st.captive;if(isCaptive){$('ssid').style.display='block';$('ssidManual').style.display='none';scan()}else{$('ssid').style.display='none';$('ssidManual').style.display='block';$('ssidManual').value=savedSsid}renderStatus(st)}catch(e){setBadge('statusMode','Unavailable','bad')}}
 $('save').onclick=async()=>{const setup=isCaptive?($('ssid').value||$('ssidManual').value.trim()):$('ssidManual').value.trim();const serverUrl=$('serverUrl').value.trim();const body={server_url:serverUrl,tailscale_enabled:$('tailscaleEnabled').checked,tailscale_name:$('tailscaleName').value,wifi_ssid:setup,wifi_password:$('wifiPassword').value,admin_password:$('adminPassword').value,tailscale_auth_key:$('tailscaleKey').value};try{let r;if(isCaptive&&setup){r=await fetch('/connect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid:setup,pswd:$('wifiPassword').value,server:serverUrl,...body})})}else{r=await authFetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})}msg(await r.text());if(r.ok&&setup) setTimeout(()=>location.reload(),1500)}catch(e){msg('Save failed: '+e)}};
 $('restart').onclick=async()=>{try{const r=await authFetch('/api/restart',{method:'POST'});msg(await r.text())}catch(e){msg('Restart failed: '+e)}};
 $('factoryReset').onclick=async()=>{if(!confirm('Factory reset this frame? This deletes all saved Wi-Fi, server, portal-password, refresh-rate, and Tailscale settings.'))return;try{const r=await authFetch('/api/factory-reset',{method:'POST'});msg(await r.text());if(r.ok)setTimeout(()=>location.href='/',3000)}catch(e){msg('Factory reset failed: '+e)}};
...
input,button{box-sizing:border-box;width:100%;padding:.7rem;margin-top:.4rem;font:inherit}button{margin-top:1.2rem;background:#146c94;color:#fff;border:0;border-radius:4px}.muted{color:#666;font-size:.95rem}#message{margin-top:1rem;white-space:pre-wrap}.password-toggle{display:flex;align-items:center;gap:.45rem;margin-top:.45rem;font-size:.9rem}.password-toggle input{width:auto;margin:0}
</style></head><body>
...
<label for="password">Admin password</label><input id="password" type="password" autocomplete="new-password" minlength="8" maxlength="16" placeholder="8 to 16 characters"><label class="password-toggle"><input id="showPassword" type="checkbox"> Show password</label>
<label for="confirm">Confirm admin password</label><input id="confirm" type="password" autocomplete="new-password" minlength="8" maxlength="16"><label class="password-toggle"><input id="showConfirm" type="checkbox"> Show confirmation</label>
<button id="save">Create password</button>
<p id="message"></p>
<script>
const $=id=>document.getElementById(id),msg=t=>$('message').textContent=t;
function bindPasswordToggle(toggleId,inputId){const toggle=$(toggleId),input=$(inputId);toggle.onchange=()=>{input.type=toggle.checked?'text':'password'}}
bindPasswordToggle('showPassword','password');bindPasswordToggle('showConfirm','confirm');
$('save').onclick=async()=>{const p=$('password').value;if(p.length<8||p.length>16){msg('Use 8 to 16 characters.');return}if(p!==$('confirm').value){msg('The passwords do not match.');return}try{const r=await fetch('/api/bootstrap',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({admin_password:p})});const body=await r.text();if(!r.ok){msg(body);return}sessionStorage.setItem('epfAdminPassword',p);msg('Password created. Loading settings…');setTimeout(()=>location.href='/',600)}catch(e){msg('Could not create password: '+e)}};
</script></body></html>
)rawliteral";
